#!/bin/sh
set -e
# Code IS NOT generated by godownloader. Go ahead and edit it.
#
...
execute() {
  ...
  install -d "${BINDIR}"
  for srcbinexe in "${srcdir}"/*; do
    binexe=${srcbinexe##*/}
    # Use the case statement to filter out files that shouldn't be installed.
    case "${binexe}" in
    [A-Z]*)
      echo "Skipping $binexe"
      continue
      ;;
    esac
    install "${srcbinexe}" "${BINDIR}/"
